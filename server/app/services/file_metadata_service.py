...
async def update_knowledge(db: AsyncSession) -> bool:
    ...
    db_files_result = await db.execute(select(FileMetaData))
    db_files = db_files_result.scalars().all()

    mapping = {file.id: {"version": file.version, "delete": True} for file in db_files}
    ...
    for file in files_in_drive:
        if file.id in mapping:
            ...
            files_for_deletion.append(
                FileMetaData(id=file.id, version=file.version, mime_type=file.mime_type)
            )
            files_for_embedding.append(
                FileMetaData(
                    id=file.id,
