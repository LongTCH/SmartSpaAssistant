...
class MessageRewriteAgent:
    CONFIG = {
        "model_retries": 0,
    }

    @observe(as_type="generation", name="message_rewrite_agent")
    async def run(
        self,
        user_prompt: str,
        deps: None = None,
        ...
        while True:
            try:
                agent_response: str = await b.MessageRewriteAgent(
...
message_rewrite_agent = MessageRewriteAgent()
